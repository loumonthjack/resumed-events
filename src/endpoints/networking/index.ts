...
eventRouter.get('/attendee/:eventId/notify', async (req: Request, res: Response) => {
...
eventRouter.post('/attendee/:eventId/create', validEventMiddleware, async (req: Request, res: Response) => {
...
eventRouter.get('/attendee/:eventId/share/:attendeeId', validEventMiddleware, async (req: Request, res: Response) => {
    const eventInfo = await getEventInfo(req.params.eventId);
...
eventRouter.get('/update/:eventId/:code', async (req, res, next) => {
    const eventInfo = await getEventInfo(req.params.eventId);
    if (!eventInfo) {
        return sendErrorTemplate(res, 'Event not found');
    }
...
eventRouter.post('/update/:eventId/:code', async (req, res, next) => {
    const eventInfo = await getEventInfo(req.params.eventId);
    if (!eventInfo) {
        return sendErrorTemplate(res, 'Event not found');
    }
...
eventRouter.get('/share/:eventId/start', validEventMiddleware, async (req: Request, res: Response) => {
    const eventInfo = await getEventInfo(req.params.eventId);
    return $sendEventLandingTemplate(req, res, eventInfo);
});

// show attendee code share page
eventRouter.get('/share/:eventId/portal', validEventMiddleware, async (req: Request, res: Response) => {
    const eventInfo = await getEventInfo(req.params.eventId);
    const eventAttendants = await prisma.eventAttendant.findMany({
